...
j1396:		sec					//1396
			lda		X_h + SHIP
			sbc		X_h,x
			php		// save N	MISSING LDY #$03 (needed?)
			lda		#80
			plp		
			bpl		+
			ldy		#$fd		//-3
...
+			lda		_srcptr_l	//1c19
			...
			beq		target_ret
			lda		dX_l,y		//+dx
++			cmp		_shootspeed	//1c22 DIFF NO LABEL
			bcs		target_ret
...
flashframe:	lda		#0			//stop ship
			...
			//mm_update(SHIP)
			//rts
to_pausquit:jmp		pause_quit //DIFF
			//key_fire()
			//keys_nav()
			//scroll_scrn()
			//move_ship()
...
nxlev4:		clc		
			...
			sta		Spawnc + MAN
nxlev5:		ldx		#Uf			//id #	DIFF should be #ID_MAX
			lda		#EMPTY		//ff
...
blackpal:	jsr		set_palette
			...
			lda		#(PALX_ENEMYB|BLACK) DIFF error? maybe PAL_SHIP2? instead
...
+			lda		#'\r'		//2873 0d
			sta		HiScore+3,x3 //blank DIFF ERROR x3?
			lda		_score_lsb
			sta		HiScore,x
			lda		_score_100
			sta		HiScore+1,x
